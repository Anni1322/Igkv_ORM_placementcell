...
// Change POST to GET
router.get('/getAllApplications/:studentId', studentController.getAllApplications);

router.post('/uploadNotice_Url', upload.single('Notice_Url'), (req, res, next) => {
  const file = req.file;
  if (!file) {
...
router.post('/uploadmarksheet', upload.single('Marksheet_Url'), (req, res, next) => {
  ...
  res.json({ Marksheet_Url: `/uploads/${file.filename}`});
});
