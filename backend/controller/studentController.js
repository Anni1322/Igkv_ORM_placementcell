...
const forgetPassword = async (req, res) => {
    ...
    console.log("Request from frontend:", username, UID, newpassword);
    try {
        const request = new sql.Request();
        const usernameCheckQuery = `
        SELECT l.id, l.Emp_Id, l.username
        FROM dbo.login_table AS l
        WHERE l.username = @username`;
        ...
        if (result.recordset.length > 0) {
            ...
            // Check if the provided UID matches the Emp_Id from the database
            if (UID === user.Emp_Id) {
                // Proceed to update the password
                const hashedNewPassword = await bcrypt.hash(newpassword, 10);

                const updatePasswordQuery = `
                UPDATE dbo.login_table
                ...
                // UID does not match
                return res.status(400).json({ error: 'Incorrect UID' });
            }
        } else {
            // Username does not exist
            return res.status(400).json({ error: 'Username does not exist' });
        ...
        console.error('Error during password reset in SQL Server:', error);
        return res.status(500).json({ error: 'Internal Server Error' });
    }
};
...
const postbasicdetails = async (req, res)=>{
        ...
        // console.log("in bacnek url",profilephoto_Url)
        ...
        try {
            ...
            request.input('Student_First_Name_H', sql.NVarChar(50), Student_First_Name_H);
            request.input('Student_Middle_Name_H', sql.NVarChar(50), Student_Middle_Name_H);
            request.input('Student_Last_Name_H', sql.NVarChar(50), Student_Last_Name_H);
            ...
            request.input('Father_Name_H', sql.NVarChar(50), Father_Name_H);
            request.input('Mother_Name_H', sql.NVarChar(50), Mother_Name_H);
            request.input('Guardian_Name_E', sql.VarChar(50), Guardian_Name_E);
            request.input('Spouse_Name_E', sql.VarChar(50), Spouse_Name_E);
            request.input('profilephoto_Url', sql.VarChar(2000), profilephoto_Url);
    ...
    try {
        ...
        const insertQuery = `
            INSERT INTO tnp_student_application_details 
                (Vacancy_ID, Company_ID, Company_Name, Student_ID, Full_Name, Post_Name, Fathers_Name, Email, Mobile, Status, Resume_Path, Created_Date)
            VALUES 
                (@Vacancy_ID, @Company_ID, @Company_Name, @Student_ID, @Full_Name, @Post_Name, @Fathers_Name, @Email, @Mobile, @Status, @Resume_Path, @Created_Date)`;
...
const getAllApplications = async (req, res) => {
    try {
        ...
        // Pass the Student_ID parameter to the query
        const query = `
            SELECT 
                Vacancy_ID, 
                Company_ID, 
        ...
        // Add the input parameter for the student ID
        ...
        if (result.recordset.length > 0) {
            ...
            // Respond with a 404 if no record was found for the given Student_ID
    ...
    try {
        ...
        const insertQuery = `
            INSERT INTO VacancyApplication 
                (Vacancy_ID, Company_ID, Company_Name, Student_ID, Full_Name, Post_Name, Fathers_Name, Email, Mobile, Status, Application_Submission_Date)
            VALUES 
                (@Vacancy_ID, @Company_ID, @Company_Name, @Student_ID, @Full_Name, @Post_Name, @Fathers_Name, @Email, @Mobile, @Status, @Application_Submission_Date)`;
...
const SkillDetails = async (req, res) => {
    ...
    try {
      ...
      var request = new sql.Request();
        ...
        request.input('Skill_Certificate_Url', sql.VarChar(1000), Skill_Certificate_Url )
       // request.input('Created_Date', sql.Date, Created_Date)
        const result = await request.query(`
          INSERT INTO tnp_student_skills (Student_ID, Registration_No, Skill_Id, Skill_Certificate_Url)
          VALUES (@Student_ID, @Registration_No, @Skill_Id, @Skill_Certificate_Url);
        `);
